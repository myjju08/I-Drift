...
from drifting_core.double_drift import (
    sample_double_drift_loss,
...
def compute_drift_loss_from_features(
    ...
    if (double_drift_c0, double_drift_c1) != (1.0, 0.0) and mode != "rev-drift":
        raise ValueError("double drift currently supports rev-drift only")
...
def train_step(
    ...
    double_drift_mode = str(cfg.get("double_drift_mode", "off"))
    if double_drift_mode not in ("off", "feature", "sample"):
        raise ValueError("double_drift_mode must be off, feature, or sample")
    double_c0, double_c1 = validate_double_drift_coefficients(
        cfg.get("double_drift_c0", 0.75), cfg.get("double_drift_c1", 0.25)
    )
    if double_drift_mode != "off":
        if drift_matching != "rev-drift":
            raise ValueError("double drift currently supports rev-drift only")
        if any(branch is not None for branch in (
            feature_adapter, feature_discriminator, adversarial_system,
        )):
    ...
    loss, loss_info = compute_drift_loss_from_features(
        gen_feats=gen_feats,
        **drift_loss_kwargs,
        double_drift_c0=double_c0 if double_drift_mode == "feature" else 1.0,
        double_drift_c1=double_c1 if double_drift_mode == "feature" else 0.0,
    )
    if double_drift_mode == "sample":
        def loss_at_moved_samples(moved_samples):
            with _amp_ctx(mae_use_bf16):
                moved_feats = feature_extractor.get_activations(
                    moved_samples, **act_kwargs,
                    stage_adapters=target_stage_adapters,
                )
            # Reuse the same real features, labels, CFG weights, and feature
            # selection. The current-negative pool is rebuilt from moved_feats.
            second_kwargs = dict(drift_loss_kwargs)
            second_kwargs["compute_raw_winner_stats_flag"] = False
            second_kwargs["compute_wpos_stats"] = False
            return compute_drift_loss_from_features(
                gen_feats=moved_feats, **second_kwargs
            )

        loss, sample_info = sample_double_drift_loss(
            gen_samples, loss, loss_at_moved_samples,
            c0=double_c0, c1=double_c1,
            step_rms=float(cfg.get("double_drift_sample_step_rms", 0.1)),
            global_stats=global_fnorm_stats,
        )
        loss_info.update(sample_info)
    if double_drift_mode != "off":
        loss_info["double_drift/c0"] = double_c0
        loss_info["double_drift/c1"] = double_c1
...
def main() -> None:
    ...
    parser.add_argument("--double_drift_mode", choices=("off", "feature", "sample"), default=None)
    ...
    for key in ("double_drift_mode", "double_drift_c0", "double_drift_c1", "double_drift_sample_step_rms"):

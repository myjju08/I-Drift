...
@pytest.fixture
def _no_tqdm_monitor(monkeypatch):
    """Retire prior tests' progress monitor before exercising a real fork.

    Closed tqdm bars can leave their shared daemon alive for the interpreter's
    lifetime. Stop only that known test-global monitor; unknown threads still
    reach the production single-thread-parent rejection unchanged.
    """
    monkeypatch.setattr(tqdm, "monitor_interval", 0)
    monitor = tqdm.monitor
    if monitor is not None:
        assert isinstance(monitor, TMonitor)
        monitor.exit()  # Signals and joins the known monitor thread.
        assert not monitor.is_alive()
        tqdm.monitor = None
    yield
    # monkeypatch restores monitor_interval; a later bar may create a fresh one.
...
def test_process_executor_real_range_http_and_verified_resume(tmp_path, monkeypatch, mismatched_hash, use_affinity, _no_tqdm_monitor):
...
def test_actual_fork_worker_affinities_are_assigned_once_and_parent_unchanged(_no_tqdm_monitor):

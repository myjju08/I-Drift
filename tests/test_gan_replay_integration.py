"""Check GAN integration with the trainer's existing replay and DINO APIs."""
...
class _StrictFrozenDino(_TinyFrozenDino):
    # Keep the fake encoder API as strict as the real frozen DINO extractor.
    def get_activations(self, images, *, return_stage_features=False):
...
class GanReplayIntegrationTest(unittest.TestCase):
    ...
    def test_mixed_dino_uses_supported_stage_feature_api(self):
    ...
    def test_full_mixed_loop_wires_uint8_banks_checkpoint_and_resume(self):
        ...
        cfg = {
            **_trainer_inputs()["cfg"],
            ...
            "push_at_resume": 1, "total_steps": 3, "train_max_step_exclusive": 1,
            "eval_at_start": False, "save_per_step": 1,
        ...
        def run(config, workdir):
            banks = []
            bank_class = trainer.CompressedPixelMemoryBank

            class TrackedBank(bank_class):
            ...
            with (
                ...
                mock.patch.object(trainer, "CompressedPixelMemoryBank", TrackedBank),
            ):
                try:
        ...
        with tempfile.TemporaryDirectory(prefix="gan-replay-integration-") as workdir:
            ...
            train_kwargs = [kwargs for kwargs in loader_kwargs if kwargs["split"] == "train"]
            self.assertEqual(len(train_kwargs), 2)
            self.assertTrue(all(kwargs["return_uint8"] for kwargs in train_kwargs))
            self.assertTrue(all(not kwargs.get("return_uint8", False)
                                for kwargs in loader_kwargs if kwargs["split"] == "val"))

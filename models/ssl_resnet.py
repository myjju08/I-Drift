"""Frozen DINO ResNet-50 features for direct RGB drifting.

Encoder weights stay frozen while gradients flow to generated pixels. Feature
names, pooling and spatial statistics retain the original DINO training math.
"""
...
from models.feature_statistics import safe_mean, safe_mean_std, safe_rms, safe_std
...
def derive_ssl_map_features(
    ...
    *,
    ...
    """Derive DINO map statistics shared by drifting and offline DINO tuning."""
...
def canonical_ssl_backbone(name: str) -> str:
    ...
    if normalized not in aliases:
        raise ValueError(
            f"Unknown feature backbone {name!r}; choose dino_resnet50"
        )
...
def _load_backbone_state(path: str | Path, backbone_name: str) -> Dict[str, torch.Tensor]:
    ...
    if canonical_ssl_backbone(backbone_name) != "dino_resnet50":
...
class SSLResNetFeatureExtractor(nn.Module):
    """Frozen DINO ResNet-50 with differentiable RGB input normalization.

    ``spatial_pool=2`` maps the 256px grids ``64/64/32/16/8`` to
    ``32/32/16/8/4``, preserving the established DINO token geometry.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if use_latent:
            raise ValueError("DINO feature extraction supports direct RGB inputs only")
        ...
        # Bottleneck ResNet stage widths are 256, 512, 1024, 2048.
    ...
    def _decode_and_normalize(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 4 or x.shape[1] != 3:
            raise ValueError(f"DINO expects BCHW RGB inputs, got {tuple(x.shape)}")
        pixels = x
    ...
    def get_activations(
        ...
        # views whenever both are requested. Process the shared activation
        # once and publish the same derived tensors under
        ...
        for start in range(0, int(x.shape[0]), chunk_size):
            ...
            for name, feature in zip(names, maps):
                # Adversarial structure preservation consumes terminal maps.
                if return_stage_features and name in {"layer1", "layer2", "layer3", "layer4"}:
